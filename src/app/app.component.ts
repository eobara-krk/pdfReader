function customReplacements(text: string): string {
  ...
  .replace(/\bSS\b/g, 'spesalvi')
  .replace(/spe salvi/gi, 'spesalvi')
  .replace(/Cześć/g, 'Część')
  .replace(/\bSnC\b/g, 'spes non confundit')
  .replace(/2025 roku/g, 'dwa tysiące dwudziestego piątego roku')
  .replace(/1994 roku/g, 'tysiąc dziewięćset dziewięćdziesiątym czwartym roku')
  .replace(/Jubileusz Roku 2025/g, 'Jubileusz Roku dwa tysiące dwudziestego piątego')
  .replace(/9\.05\.2024 roku/g, 'dziewiątego maja dwatysiące dwudziestego czwartego roku')
  .replace(/\bBT5\b/g, 'Biblia Tysiąclecia, wydanie piąte')
  .replace(/14\.01\.2025 roku/g, 'czternastego stycznia 2025 roku')
  .replace(/Leon XIV/g, 'Leon czternasty')
  .replace(/por\./gi, '')
  .replace(/\bMk\b/g, 'Marek')
  .replace(/11\.06\.2025 roku/g, 'jedenasty czerwca dwa tysiące dwudziestego piątego roku')
  .replace(/Roku Jubileuszowym 2025/g, 'roku jubileuszowym dwa tysiące dwudziestym piątym')
  .replace(/4–10 sierpnia 2025 roku/g, 'od czwartego do 10 sierpnia dwa tysiące dwudziestego piątego roku')
  .replace(/\bEf\b/g, 'efezjan')
  .replace(/\bTm\b/g, 'Tymoteusz')
  .replace(/\bJ\b/g, 'Jan')
  .replace(/https?:\/\/tiny\.pl\/\S+/gi, 'odnośnik w pedeefie')
  .replace(/https?:\/\/\S+/gi, 'należy odsłuchać katechezy, link w pedeefie')
      .replace(/<a [^>]*href=["']https?:\/\/[^"']+["'][^>]*>.*?<\/a>/gi, 'należy odsłuchać katechezy, link w pedeefie')
    .replace(/1\. Listu do Tymoteusza/gi, 'pierwszego listu do tymoteusza')
  .replace(/SPE SALVI  BENEDYKTA XVI/gi, 'spesalvi benedykta szesnastego')
  .replace(/\bTes\b/gi, 'tesaloniczan')
  .replace(/Kliknij TUTAJ, aby obejrzeć video z katechezą\./gi, 'należy odsłuchać katechezy link w pedeefie');
